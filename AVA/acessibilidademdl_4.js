...
(function () {
  ...
  // ===== Persistência ========================================================
  // localStorage é o meio principal, mas em alguns cenários (modo privativo,
  // página dentro de iframe, storage bloqueado por política do navegador) ele
  // lança exceção ou é limpo a cada navegação. Nesses casos as preferências
  // "sumiam" ao trocar de página. O cookie abaixo é o plano B e ainda cobre
  // navegação entre subdomínios do mesmo site.
  const COOKIE_MAX_AGE = 60 * 60 * 24 * 365; // 1 ano

  function cookieDomain() {
    const parts = location.hostname.split('.');
    if (parts.length < 3) return '';           // localhost / dominio.com
  ...
  function writeCookie(value) {
    try {
      document.cookie = STORAGE_KEY + '=' + encodeURIComponent(value) +
        '; path=/; max-age=' + COOKIE_MAX_AGE + '; SameSite=Lax' + cookieDomain();
    } catch (e) {}
  }
  ...
  function loadPrefs() {
    try {
      ...
      prefs.darkModeEngine = normalizeEngine(prefs.darkModeEngine);
      prefs.fontScale = clamp(Number(prefs.fontScale) || 1, 0.85, 1.35);
      prefs.grayscaleScale = clamp(Number(prefs.grayscaleScale) || 100, 0, 100);
  ...
  function savePrefs() {
    ...
    try {
      ...
      ok = localStorage.getItem(STORAGE_KEY) === value;
    } catch (e) {}
    // Grava sempre também no cookie: se o localStorage falhar (ou for limpo),
    // a configuração continua valendo na próxima página.
    writeCookie(value);
    return ok;
  }
...
<div class="a11y-bar" role="region" aria-label="Barra de acessibilidade">
  ...
  function init() {
    ...
    // Restaura TUDO imediatamente (fonte, cinza, movimento, foco, barra
    // recolhida e modo noturno). Antes, quando o modo noturno estava salvo, o
    // applyPrefs só rodava dentro do callback do DarkReader — se o CDN fosse
    // bloqueado (CSP do Moodle) o callback nunca disparava e NENHUMA
    // preferência era reaplicada, dando a impressão de que nada era salvo.
    applyPrefs();
    // Reaplica após o Moodle terminar de montar a página, sem depender do CDN.
    [60, 400, 1200].forEach(delay => setTimeout(applyPrefs, delay));

    // Última chance de gravar antes de sair da página (navegação no Moodle).
